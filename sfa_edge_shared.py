...
"""
SFA Edge Shared - Shared User Profile Edge Automation

This tool is a variant of sfa_edge_ai.py designed to run with a specific
shared user profile ("user-ai") on a dedicated port (9225).

It automatically launches Edge if it's not running.
"""
...
PROFILE_DIR_NAME = "edge_profiles/user-ai"

def get_profile_path():
    # Create a local 'edge_profiles' directory in the toolkit folder
    base_dir = Path(__file__).parent / "edge_profiles"
    return base_dir / "user-ai"

def launch_edge(port: int = DEFAULT_PORT):
    """Launch Edge with the shared profile if not already running."""
...
class CDPClient:
    """Base client for Edge DevTools Protocol interactions."""
    ...
    def send_command(self, method: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if not self.ws:
            raise RuntimeError("Not connected to Edge")
...
def main():
    parser = argparse.ArgumentParser(description="SFA Edge Shared - Shared User Profile Automation")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help="Edge remote debugging port")
