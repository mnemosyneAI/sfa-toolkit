...
DISPLAY_CHAR_LIMIT = 25000  # System truncates at 30k; leave buffer
...
def read_file(
    ...
    For large files exceeding DISPLAY_CHAR_LIMIT, shows head + tail
    with a warning about skipped middle section.
    """
    ...
    try:
        ...
        # Check if output exceeds display limit
        if len(output) > DISPLAY_CHAR_LIMIT:
            # Calculate how many chars for head and tail (leave room for warning)
            section_size = (DISPLAY_CHAR_LIMIT - 500) // 2
            
            # Find line boundaries for clean cuts
            head_lines = []
            head_chars = 0
            for line in output_lines:
                if head_chars + len(line) + 1 > section_size:
                    break
            ...
            for line in reversed(output_lines):
                if tail_chars + len(line) + 1 > section_size:
                    break
                tail_lines.insert(0, line)
                tail_chars += len(line) + 1
            
            # Calculate skipped section
            head_end = start_idx + len(head_lines)
            tail_start = start_idx + len(selected_lines) - len(tail_lines) + 1
            skipped_lines = tail_start - head_end - 1
            
            warning = f"\n\n{'='*60}\n⚠ LARGE FILE: {total_lines} total lines, {len(output)} chars\n"
            warning += f"  Showing lines 1-{head_end} and {tail_start}-{total_lines}\n"
            warning += f"  SKIPPED {skipped_lines} lines ({head_end+1}-{tail_start-1})\n"
            warning += f"  Use: fs read [file] --offset {head_end} --limit {skipped_lines}\n"
            warning += f"{'='*60}\n\n"
            
            output = "\n".join(head_lines) + warning + "\n".join(tail_lines)
